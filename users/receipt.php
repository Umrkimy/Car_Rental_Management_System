...
if (isset($_GET['bookingid'])) {
    ...
    if (!isset($_SESSION['inserted_booking'])) {
        $sql = "INSERT INTO bookings (full_name, user_name, client_name, email, phone_num, cars_name, ic_no, driver_no, days_rented, deposit, total, status, pickup_location, dropoff_location, pickup_date, dropoff_date, invoice_no, invoice_date) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssssisssssssss", 
            $full_name, $user_name, $clientname, $email, $phone, 
            $cars_name, $ic_no, $driver_no, $days_rented, $deposit_rm, 
            $total_rm, $status, $pickup_location, $drop_location, 
            $pickup_date, $drop_date, $invoice_no, $date);
